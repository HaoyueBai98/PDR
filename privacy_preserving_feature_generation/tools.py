...
#     K_centered = torch.mm(H, torch.mm(K, H))
#     return K_centered
...
#     Kc = center_kernel(K)
#     Lc = center_kernel(L)
#     hsic_value = torch.trace(torch.mm(Kc, Lc)) / (X.size(0) - 1) ** 2
#     return hsic_value
...
def hsic(X, Y, a=None):
    ...
    sigma_x = np.var(X_tensor.numpy())
    sigma_y = np.var(Y_tensor.numpy())
    
    sigma_x = max(sigma_x, 1e-5)
    sigma_y = max(sigma_y, 1e-5)

    try:
        K = torch.tensor(rbf_kernel(X_tensor.numpy(), gamma=1.0 / (2 * sigma_x)), dtype=torch.float32)
        L = torch.tensor(rbf_kernel(Y_tensor.numpy(), gamma=1.0 / (2 * sigma_y)), dtype=torch.float32)
    except:
        return 1
    Kc = center_kernel(K)
    Lc = center_kernel(L)
    ...
    if torch.isnan(hsic_value):
        return 0.0 
    return hsic_value.item()
# def kernel_matrix(x, sigma):
#     if x.dim() == 1:  # Check if x is a vector
#         x = x.unsqueeze(0)  # Convert vector to 1xN matrix
...
#     y_test = np.array(y_test)
#     y_predict = np.array(y_predict)
#     return np.mean(np.abs(y_test - y_predict) / ((np.abs(y_test) + np.abs(y_predict)) / 2))

def downstream_task_new(data, task_type, metric_type=None, state_num=10):
    X = data.iloc[:, :-1]
    y = data.iloc[:, -1]
    if task_type == 'cls':
...
def sensitive_test(data, task_type='reg'):
    X = data.iloc[:, :-1]
    y = data.iloc[:, -1]
    if task_type == 'cls':
        clf = RandomForestClassifier(random_state=0)
