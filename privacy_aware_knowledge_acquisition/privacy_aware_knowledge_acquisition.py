...
from tools import *
from task_mapping import task_dict, task_type, base_path, task_measure, state_rep, support_rl_method, sens_task_dict
import argparse
import wandb
...
def init_param():
    ...
    parser.add_argument('--file-name', type=str, default='german_credit', 
                        help='data name')
    ...
    parser.add_argument('--ib_coeff', type=float, default=0.3)
...
def model_train(param):
    ...
    D_OPT_PATH = './tmp_privacy/' + param['file_name'] + '/'
    ...
    Dg_wo_sen = Dg.iloc[:, 1:]
    old_per = downstream_task_new(Dg_wo_sen, task_name, measure, state_num=22)
    ...
    while episode < EPISODES:
        ...
        while step < STEPS:
            ...
            if Dg.shape[1] > FEATURE_LIMIT:
                selector = SelectKBest(mutual_info_regression, k=FEATURE_LIMIT).fit(Dg.iloc[:, 1:-1], Dg.iloc[:, -1])
                cols = selector.get_support()
                cols_adjusted = np.insert(cols, 0, True)
                X_new = Dg.iloc[:, :-1].loc[:, cols_adjusted]
                Dg = pd.concat([X_new, Dg.iloc[:, -1]], axis=1)
                
            feature_names = list(Dg.columns)
            try:
                Dg_wo_sen = Dg.drop(columns='21', inplace=False)
            except:
                Dg_wo_sen = Dg
            new_per = downstream_task_new(Dg_wo_sen, task_name, measure, state_num=0)
            sensitive_per = sensitive_test(pd.concat([Dg_wo_sen.iloc[:, :-1], sensitive], axis=1), task_type = sens_task_dict[param['file_name']])
            

            generated_feature_state = torch.tensor(feature_state_generation(f_generate, STATE_METHOD, DEVICE), dtype=torch.float32).reshape(-1,1)
            sensitive_feature_state = torch.tensor(feature_state_generation(pd.DataFrame(sensitive), STATE_METHOD, DEVICE), dtype=torch.float32).reshape(-1,1)

            # input_x = F.normalize(generated_feature_state, p=2, dim=-1)
            # input_y = F.normalize(sensitive_feature_state, p=2, dim=-1)
            # Kx = kernel_matrix(input_x, 0.25)
            # Ky = kernel_matrix(input_y, 0.25)
            mi_hb = - hsic(generated_feature_state, sensitive_feature_state, 0.25)

            reward =  (1 - param['ib_coeff']) * (new_per - old_per) + param['ib_coeff'] * mi_hb
            # reward =  param['ib_coeff'] * mi_hb

            wandb.log({'reward/new_per - old_per':new_per - old_per, 'reward/mi_hb':mi_hb}, step = total_step)

            wandb.log({'train/reward':reward, 'train/performance':new_per, 'train/sensitive_per':sensitive_per}, step = total_step)
            r_c1, r_op, r_c2 = param['a'] / 10 * reward / 3, param['b'
            ] / 10 * reward / 3, param['c'] / 10 * reward / 3

            if sensitive_per < best_sensitive:
                wandb.log({'best_sens/performance_when_best_sen':new_per, 'best_sens/best_sensitive_per':sensitive_per}, step = total_step)
                best_sensitive = sensitive_per 
            if new_per > best_per:
                wandb.log({'best_p/best_performance':new_per, 'best_p/sen_when_best_p':sensitive_per, 'best_p/hsic':-mi_hb}, step = total_step)

                always_best.append((Dg_wo_sen.columns, -mi_hb, new_per, episode, step))
                best_episode = episode
                best_per = new_per
                D_OPT = Dg_wo_sen.copy()
            if new_per > local_best:
                local_best = new_per
                Dg_local = Dg_wo_sen.copy()
            all.append((Dg_wo_sen.columns, -mi_hb, new_per, episode, step))
            old_per = new_per
        ...
        if episode % 5 == 0:
            # wandb.log({'best_perform':np.min(best_per_opt)}, step = total_step)
    ...
    D_OPT.to_csv(D_OPT_PATH + '/' + f'{wandb.run.name}_{best_per}.csv')
    always_best_df = []
    with open(D_OPT_PATH + '/' + f'{wandb.run.name}.bdata', 'w') as f:
        for col_name, mi_wp, per, epi, step_ in always_best:
            col_name = [str(i) for i in list(col_name)]
            line = str.join(f',{str(sep_token)},', col_name) + f',{mi_wp},{per},{epi},{step_}\n'
            f.write(line)
    with open(D_OPT_PATH + '/' + f'{wandb.run.name}.adata', 'w') as f:
        for col_name, mi_wp, per, epi, step_ in all:
            col_name = [str(i) for i in list(col_name)]
            line = str.join(f',{str(sep_token)},', col_name) + f',{mi_wp},{per},{epi},{step_}\n'
            f.write(line)
...
if __name__ == '__main__':
    try:
        ...
        wandb.init(project="",config=params)

        torch.manual_seed(params['seed'])
        torch.cuda.manual_seed(params['seed'])

        if not os.path.exists('./tmp_privacy'):
            os.mkdir('./tmp_privacy/')
        if not os.path.exists('./tmp_privacy/' + params['file_name'] + '/'):
            os.mkdir('./tmp_privacy/' + params['file_name'] + '/')
